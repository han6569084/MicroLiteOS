...
{
    printf("Configuring DMA2D transfer...\r\n");

    /* number of lines register */
    // DMA2D->NLR = (conf->w << DMA2D_NLR_PL) | (conf->h << DMA2D_NLR_NL);
    DMA2D->NLR = ((conf->w & 0x3FFF) << 16) | (conf->h & 0xFFFF);

    printf("conf = %d %d\r\n", conf->w, conf->h);
    printf("DMA2D->NLR=0x%08lx\r\n", DMA2D->NLR);
    ...
    DMA2D->OPFCCR = ((uint32_t) conf->output_cf) << DMA2D_OPFCCR_CM;
    ...
    DMA2D->FGPFCCR = (((uint32_t) conf->fg_cf) << DMA2D_FGPFCCR_CM)
                     | (conf->fg_alpha << DMA2D_FGPFCCR_ALPHA)
                     | (conf->fg_alpha_mode << DMA2D_FGPFCCR_AM);
    ...
    DMA2D->BGPFCCR = (((uint32_t) conf->bg_cf) << DMA2D_BGPFCCR_CM)
                     | (conf->bg_alpha << DMA2D_BGPFCCR_ALPHA)
                     | (conf->bg_alpha_mode << DMA2D_BGPFCCR_AM);
    ...
    DMA2D->CR = DMA2D_CR_START | (((uint32_t) conf->mode) << DMA2D_CR_MODE)
#if LV_USE_DRAW_DMA2D_INTERRUPT
                | DMA2D_CR_TCIE
#endif
                ;
...
static int32_t dispatch_cb(lv_draw_unit_t * draw_unit, lv_layer_t * layer)
{
    ...
    printf("Dispatching draw task to draw unit %p\r\n", draw_unit);
